...
[excited] Everything in AI DevKit runs through six markdown files in a prompts folder. [short pause] spec, implement, create docs, update docs, verify with Codex, verify with Claude. [pause] They're plain text — you can read them, understand exactly what the AI is being told to do, and modify them. [pause] [deliberate] Why not skills or slash commands? [short pause] There are lots of ways to load instructions into an AI tool — user settings, project files, enterprise configs, skills, slash commands. [short pause] It's a walled garden. [short pause] A Claude skill doesn't work in Cursor. [short pause] A Cursor command doesn't work in Codex. [pause] But at the end of the day, it's all just text hitting an LLM. [pause] Prompts are portable — you can paste them into any agent, or pipe them from the command line. [short pause] Curl the prompts straight from GitHub — no clone needed. [pause] For example: [short pause] curl create-docs and verify-codex, pipe to claude. [short pause] Or flip it — curl create-docs and verify-claude, pass to codex. [pause] Either way, one command generates Progressive Disclosure docs and runs cross-model verification — end to end, about fifty minutes, zero human intervention. [pause] [deliberate] Visible prompts are accountable. Hidden skills may or may not be followed. [short pause] Engineers should read the prompts, not trust hidden instructions.

## Slide 8: Getting Started

[excited] Here's how to start. [pause] Pick one repo. [short pause] Curl create-docs, pipe to claude — or pass to codex. [short pause] That's it — any agent can then pick up that repo and work safely. [pause] Chain verify-codex or verify-claude for cross-model review — works in either direction. [pause] The repo, the standard, and all the prompts are open at github.com/AgoraIO-Community/ai-devkit.
