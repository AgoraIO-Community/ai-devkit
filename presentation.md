...
Progressive Disclosure docs give every repo the same structure. [pause] [deliberate] The same eight files, in every repo, across the whole organisation. [pause] Level zero is an identity card — what is this repo? [short pause] Level one is eight fixed files: [short pause] setup, architecture, code map, conventions, workflows, interfaces, gotchas, and security. [pause] Level two is deep dives, loaded only when level one isn't enough. [pause] Levels zero and one load upfront — they're small. [short pause] Level two is pulled in only when needed. [short pause] The agent gets full context without filling the context window. [pause] [deliberate] That consistency is what lets any agent pick up any repo and work safely. [pause] You can generate these docs for an existing repo today — that's the fastest way to start.

## Slide 6: Why Prompts Not Skills

[thoughtful] Why prompts instead of skills or slash commands? [short pause] There are lots of ways to load instructions into an AI tool — user settings, project files, enterprise configs, workspace markdown, skills, slash commands. [short pause] It's a walled garden. [short pause] A Claude Code skill doesn't work in Cursor. [short pause] A Cursor command doesn't work in Codex. [pause] But at the end of the day, it's all just text hitting an LLM. [pause] Prompts are portable — they work in any agent that can read files. [short pause] The same prompt works in Claude Code, Cursor, Codex, Gemini, or whatever comes next. [pause] [deliberate] More importantly, a prompt is visible text. [short pause] Engineers can read the prompt, understand exactly what the AI is being told to do, and modify it. [short pause] Hidden skills may or may not be followed. [short pause] Visible prompts are accountable. [pause] When a workflow changes, you update the markdown file, [short pause] and every prompt that references it picks up the change. [pause] If your tool supports custom commands, [short pause] you can wrap the prompt for faster access. [short pause] [deliberate] But the prompt is the source of truth.

## Slide 7: Dev Environment
...
[excited] Here's how to start. [pause] Pick one repo and generate Progressive Disclosure docs for it. [short pause] Any agent can then pick up that repo and work safely. [pause] Next, stand up one dev environment for a multi-repo system. [short pause] The AI can run it end to end from day one. [pause] The docs and prompts are plain markdown — no plugins, no vendor lock-in. [pause] [deliberate] Start with one repo and one system, and you can use this operating model now.
